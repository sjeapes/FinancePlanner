...
def _register_routers(app: FastAPI) -> None:
    ...
    try:
        ...
        try:
            from backend.api.routes import generational
            app.include_router(generational.router, prefix="/api", tags=["generational"])
            logger.debug("_register_routers: generational router registered")
        except ImportError as exc:
        ...
        # ── Phase 8: Tax optimiser ────────────────────────────────────────────
        try:
            from backend.api.routes import tax_optimiser
            app.include_router(tax_optimiser.router, prefix="/api", tags=["tax_optimiser"])
            logger.debug("_register_routers: tax_optimiser router registered")
        except ImportError as exc:
        ...
        # ── Phase 12: Insights (MC surfacing + annual review) ─────────────────
        try:
            from backend.api.routes import insights
            app.include_router(insights.router, prefix="/api", tags=["insights"])
            logger.debug("_register_routers: insights router registered")
        except ImportError as exc:
